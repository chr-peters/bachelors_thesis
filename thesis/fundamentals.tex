...
it collects.\footnote{The details of this process are further
  illustrated in section \ref{sec:artificial-neurons}.}
...
usually normalized and scaled to equal ranges before being fed into
the network.
...
rather have too many hidden neurons and hidden layers than an
...
As already hinted at in section \ref{sec:softmax} about the softmax
...
will assign to the example as a result of the classification process.
...
\(l\), we can slightly rewrite the equations that describe the input
\(z\) and the output \(y\) of a neuron \(k\) like this:
\begin{equation}
