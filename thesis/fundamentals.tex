...
The goal behind this procedure is to convert an input signal
...
\begin{figure}[h]
  ...
  \caption{The components of the neural model. This neuron labeled
...
slopes. An illustration of this relationship is presented in fig. \ref{fig:sigmoid}.
\begin{figure}[h]
  \centering
...
neural networks. A plot of the ReLU function is presented in fig. \ref{fig:relu}.
\begin{figure}[h]
  \centering
...
clear-cut values as shown in the following example:
...
separate randomly sampled batches instead. Each batch is then used to
take a step of gradient descent by
...
The last step that has to be taken before deploying a neural network
...
with a variance of \(\frac{2}{n_{in} + n_{out}}\), where \(n_{in}\) is the number
of neurons in the preceding layer and \(n_{out}\) is the number of
neurons in the following layer of the weight. This procedure is
also called ``Xavier Initialization'' and leads to improved learning
beyond just breaking the symmetry between neuron
activations by achieving a balance of the weight values that works well in
practice.\footnote{See \cite{Gloriot} for further remarks and
  explanations as well as the
