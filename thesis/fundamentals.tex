...
algorithm is to predict the label of an instance of the dataset only
based its features. In order to achieve this, the classifier first has
to build a model based on a training dataset which is called
...
teacher, classification is an example of a broader domain that is called
...
\textit{confusion matrix} that is structured as shown in \fref{fig:confusion-matrix}.
\begin{figure}[h]
  \centering
