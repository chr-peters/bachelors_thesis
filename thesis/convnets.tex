...
in the form of a two dimensional grid, where each cell resembles a
...
one pixel in the image. As was the case with ordinary feed forward
networks, the input layer does not compute an activation function and
just passes its signal further into the network.
...
height be \(y_K\) and the depth be \(z\), where \(z\) is usually equal
...
\begin{equation}
  ...
  z_k = \sum_{n=1}^{z}{\sum_{m=1}^{y_K}{\sum_{l=1}^{x_K}{X[i+l, j+m, n] \cdot
        K[l, m, n]}}} + b
\end{equation}
...
the ReLU function which results in the following expression describing the
...
fields, we will first only shift in the \(x\) direction of the
input. The amount of shifting in each step is described by the
\textit{x\_stride} of the kernel. After shifting across the \(x\) axis
is no longer possible, the \(x\) position of the local receptive field is
reset to zero and a shift happens in the \(y\) direction. This shift
can be adjusted by the parameter \textit{y\_stride}. The same
procedure is repeated until every area of the input is covered by a
neuron.
...
detected, this procedure contributes to the \textit{spacial
  invariance} of convolutional neural network. What this means it that
the network is able to recognize certain features, no matter where
exactly they are in the image. This is one of the most important traits of
the CNN architecture which greatly contributes to its effectiveness in
